...
function renderKPIs() {
    // --- 1. Calculate and display Late Permissions ---
    const thisMonth = new Date().getMonth();
    const thisYear = new Date().getFullYear();
    const latePermissionsThisMonth = myRequests.filter(r => 
        r.type === 'Late' && 
        r.status === 'Approved' && 
        new Date(r.payload.requestedDate).getMonth() === thisMonth &&
        new Date(r.payload.requestedDate).getFullYear() === thisYear
    ).length;
    ...
    // --- 2. Calculate and display WFH for the week ---
    const thisWeek = getWeekNumber(new Date());
    const wfhThisWeek = myRequests.filter(r => 
        r.type === 'WFH' && 
        r.status === 'Approved' && 
        getWeekNumber(new Date(r.payload.requestedDate)) === thisWeek &&
        new Date(r.payload.requestedDate).getFullYear() === thisYear
    ).length;
    document.getElementById('wfhKpi').textContent = `${wfhThisWeek} / ${AppConfig.WFH_QUOTA_PER_WEEK}`;
    
    // --- 3. Calculate and display Pending Requests ---
    ...
    // --- 4. Calculate and display Estimated Deductions ---
    if (payrollImpact) {
