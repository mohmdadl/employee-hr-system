...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let requests = DataService.getRequests(); // ✅ load requests (Leave, Permission, WFH)

  // ✅ Check if employee has approved request today
  function getTodayRequest(empId) {
  ...
  // ✅ Set Status & Notes based on check-in or request
  function autoUpdateStatus(record) {
    record.minutesLate = 0;
    record.notes = record.notes || "";

    // 🔹 Check requests first
    const todayReq = getTodayRequest(record.employeeId);
    if (todayReq) {
      if (todayReq.type === "Leave") {
        record.notes = "Approved Leave";
        return "Leave";
      }
      if (todayReq.type === "WFH") {
        record.notes = "Working from Home";
        return "WFH";
      }
      if (todayReq.type === "Permission") {
        record.notes = "Permission Granted";
        // Permission → still need to check check-in
      }
    }
  ...
  function renderBoard(searchTerm = "") {
    ...
    filteredEmployees.forEach((emp) => {
      ...
      tr.innerHTML = `
        ...
        }" ${record.status === "Leave" || record.status === "WFH" ? "disabled" : ""}>
        </td>
        <td>
