    ...
    setTimeout(() => {
      ...
      pendingRequests.forEach((req) => {
        ...
        tr.innerHTML = `
                    ...
                    <td class="text-truncate" style="max-width: 250px;">${req.payload.reason || "--"
          }</td>
                    <td>${req.createdAt}</td>
                    <td class="text-end">
      ...
      card.innerHTML = `
            <div class="card ${isOverdue ? "border-danger" : ""}">
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle mb-2 text-body-secondary">Priority: ${task.priority
        }</h6>
                    <p class="card-text mb-1"><strong>Assignees:</strong> ${assignees}</p>
                    <p class="card-text"><strong>Deadline:</strong> <span class="${isOverdue ? "text-danger" : ""
